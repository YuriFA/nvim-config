...
function grep_search_with_ignore()
	...
	local ignored_subfolders_table = vim.split(ignored_subfolders, ",", { plain = true })
	local ignore = {}
...
function grep_search_with_last_params()
	...
	require("telescope.builtin").live_grep({
		...
		file_ignore_patterns = vim.split(last_ignored_subfolders, ",", { plain = true }),
		ignore = {},
	})
end

keymap("n", "<leader>fi", ":lua grep_search_with_ignore()<CR>", { noremap = true, silent = true })
keymap("n", "<leader>fl", ":lua grep_search_with_last_params()<CR>", { noremap = true, silent = true })

keymap("n", "<leader>fd", ":Telescope dir live_grep<CR>", { noremap = true, silent = true })
